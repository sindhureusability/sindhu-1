...
public class web_table {
	...
	public static void main(String[] args) {
		...
		// int rows=driver.findElements(By.tagName("tr")).size(); //8 prefered only if
		// you have one single table
		System.out.println("Number of rows:" + rows); // 7

		// 2) Find total number of columns

		int cols = driver.findElements(By.xpath("//table[@name='BookTable']//th")).size();
		// int cols=driver.findElements(By.tagName("th")).size(); // dont prefer if you
		...
		for (int r = 2; r <= rows; r++) {
			for (int c = 1; c <= cols; c++) {
				String value1 = driver.findElement(By.xpath("//table[@name='BookTable']//tr[" + r + "]//td[" + c + "]"))
						.getText();
		...
		for (int r = 2; r <= rows; r++) {
			...
			if (author.equals("Amit"))
				;
			{
				String bookname = driver.findElement(By.xpath("//table[@name='BookTable']//tr[" + r + "]//td[2]"))
						.getText();
